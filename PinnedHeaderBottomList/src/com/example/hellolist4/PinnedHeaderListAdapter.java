		...
		implements PinnedHeaderListView.PinnedHeaderAdapter
{
	 ...
	 * partition header.
	 */
	public View getPinnedHeaderView(int partition, View convertView,
			ViewGroup parent)
	{
	...
	public void configurePinnedHeaders(PinnedHeaderListView listView)
	{
		...
		for (int i = size; --i > maxTopHeader;)
		{
			if (mHeaderVisibility[i])
			{
				int position = listView.getPositionAt(listHeight
						- bottomHeaderHeight)
						- headerViewsCount;
				if (position < 0)
				{
				...
				// Animate the header only if the partition is completely
				// invisible below
